...
class RTBSA(QMainWindow):
	...
	self.val2 = self.val2[elements_to_use]
        ...
        if self.ui.std_cb.isChecked():
            s=std(self.val2)
            s="{:.4}".format(s)
            self.std_text.setPos((min(self.val1)+(min(self.val1)+max(self.val1))/2)/2,min(self.val2))
        ...
        if self.ui.corr_cb.isChecked():
            ...
            self.corr_text.setText("Corr. Coefficient: {:.3}".format(correlation.item(1)))
        if self.ui.line_cb.isChecked():
            (m,b)=polyfit(self.val1,self.val2,1)   
            fitdata=polyval([m,b],self.val1)
            m="{:.4}".format(m)
            self.slope_text.setPos((min(self.val1)+max(self.val1))/2,min(self.val2))
            self.slope_text.setText('Slope: '+str(m))        
            self.fit.setData(self.val1,fitdata)
        elif self.ui.parab_cb.isChecked():
            try:
                ...
                if self.fitorder==2:
                    ...
                    self.slope_text.setText(str("{:.2e}".format(co[0]))+'x^3'+str("+{:.2e}".format(co[1]))+'x^2'+str("+{:.2e}".format(co[2]))+'x'+str("+{:.2e}".format(co[3])))
                self.slope_text.setPos((min(self.val1)+max(self.val1))/2,min(self.val2))
            except:
                pass
        self.timer.singleShot(self.updatetime,self.update_BSA_Plot)
    ...
    def update_plot_FFT(self):
        ...
        newdata[nans] = np.interp(x(nans), x(~nans), newdata[~nans]) #interpolate nans
        newdata = newdata - np.mean(newdata);
    ...
    def commonactivated(self):
        if self.abort==False:
            ...
            #QObject.connect(self.timer2, SIGNAL("timeout()"), self.on_draw)#Mechanism to fix a race condition- start a timer which allows the main loop to regain control and see that the abort variable has turned positive, and then the timer restarts the drawing process after a short time, giving the active loop time to stop.  This fixes a problem I was seeing (Gibbs noticed first) with a double plot showing up (rare occurence, only happened if the user tried to plot a blank PV name from the enter line)
    ...
    def fitorderactivated(self):
        ...
        if self.fitorder not in range(11) or self.fitorder==0:
            self.statusBar().showMessage('Really?  That is going to be useful to you?  The (already ridiculous) range is 1-10.  Hope you win a nobel prize jackass.',6000)
    ...
    def reinitialize_plot(self):#This is a mess, but it works (used if user changes number points, fit type etc.) 
        self.plot.clear()
        self.avg_text=pg.TextItem('', color=(200,200,250),anchor=(0,1))
        self.std_text=pg.TextItem('', color=(200,200,250),anchor=(0,1))
        self.slope_text=pg.TextItem('', color=(200,200,250),anchor=(0,1))
        ...
        for plotlabels in [self.avg_text,self.std_text,self.slope_text,self.corr_text]:
            self.plot.addItem(plotlabels)
        try:
            if self.ui.AvsT_cb.isChecked():#Setup for single PV plotting 
                if self.ui.common1_rb.isChecked():
                    self.device = str(self.ui.common1.currentText()+'HSTBR')
                elif self.ui.enter1_rb.isChecked():
                    self.device = str(self.ui.enter1.text()+'HSTBR')
                elif self.ui.full_cb1.isChecked():
                    self.device=str(self.ui.pv1box.currentText()+'HSTBR')
                getdata=Popen("caget "+self.device,stdout=PIPE,shell=True)
                newdata=str(getdata.communicate()).split()[2:-1]
                newdata[-1]=newdata[-1][:-4]
                newdata=[float(i) for i in newdata]#If i use Popen instead of pyepics caget, the program doesn't start lagging if you change PVs (?!?!?).  Some stupid bug in new pyepics.
                #newdata=caget(self.device)
                self.curve = pg.PlotCurveItem(newdata[2800-self.numpoints:2800],pen=1)
                self.plot.addItem(self.curve)
                self.plot.setTitle(self.device)
                if self.ui.parab_cb.isChecked():
                    ...
                    pol = poly1d(co)
                    fit=pol(self.xdata)
                    self.parab=pg.PlotCurveItem(self.xdata,fit,pen=3,size=2)
                    self.plot.addItem(self.parab)
                    if self.fitorder==2:
                        self.slope_text.setText('Peak: '+str(-co[1]/(2*co[0])))
                    elif self.fitorder == 3:
                        self.slope_text.setText(str("{:.2e}".format(co[0]))+'x^3'+str("+{:.2e}".format(co[1]))+'x^2'+str("+{:.2e}".format(co[2]))+'x'+str("+{:.2e}".format(co[3])))           
                elif self.ui.line_cb.isChecked():
                    self.xdata=[]
                    for i in range(self.numpoints):
                        self.xdata.append(i)
                    (m,b)=polyfit(self.xdata,newdata[2800-self.numpoints:2800],1)
                    ...
                    m="{:.3e}".format(m)
                    self.slope_text.setText('Slope: '+str(m))
                    self.fit=pg.PlotCurveItem(self.xdata,fitdata,pen=3)
                    self.plot.addItem(self.fit)
                ...
                if self.ui.parab_cb.isChecked():
                    try:
                        ...
                        pol = poly1d(co)
                        fit=pol(sorted(self.val1))
                        self.parab=pg.PlotCurveItem(sorted(self.val1),fit,pen=3,size=2)
                        self.plot.addItem(self.parab)
                        if self.fitorder==2:
                            self.slope_text.setText('Peak: '+str(-co[1]/(2*co[0])))
                        elif self.fitorder == 3:
                    ...
                    m="{:.3e}".format(m)
                    self.slope_text.setText('Slope: '+str(m))
                    self.fit=pg.PlotCurveItem(self.val1,fitdata,pen=3)
                    self.plot.addItem(self.fit)
            else:
                getdata=Popen("caget "+self.device,stdout=PIPE,shell=True)
                newdata=str(getdata.communicate()).split()[2:-1]
    ...
    def logbook(self):
        curr_time = datetime.now()#Shamelessly stolen from Shawn (thanks buddy).  A lot of this is probably unnecessary for my purposes but I'm too lazy to clean it up
        ...
        Popen('convert '+fileName+'.png '+fileName+'.ps', shell=True)#PyQtGraph doesn't export PS files, so convert with linux 
        sleep(0.3)
        Popen('convert '+fileName+'.png -resize 500x500 '+fileName+'.png', shell=True)
        sleep(0.35)
    ...
    def on_about(self):
        msg = """Can you read this?  If so, congratulations.  You are a magical, marvelous troll."""
